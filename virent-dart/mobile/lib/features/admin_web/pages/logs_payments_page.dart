...
class LogsPaymentsPage extends ConsumerStatefulWidget {
  const LogsPaymentsPage({super.key});
  @override
  ConsumerState<LogsPaymentsPage> createState() => _LogsPaymentsPageState();
}

class _LogsPaymentsPageState extends ConsumerState<LogsPaymentsPage> {
      ...
      data: (items) {
        ...
        return Container(
          ...
          child: Column(
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Логи платежей', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['key1', 'key2', 'key3'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'key1', label: 'Key1'), AdminField(key: 'key2', label: 'Key2')], onApply: (v) async {})),
            ...
            columnSpacing: 24,
            horizontalMargin: 12,
                    headingRowColor: WidgetStateProperty.all(adminBgLight), columns: [const DataColumn(label: Text('')), const DataColumn(label: Text('Key1')), const DataColumn(label: Text('Key2')), const DataColumn(label: Text('Key3')), const DataColumn(label: Text('Действия'))], rows: pageItems.map<DataRow>((i) => _buildRow(context, ref, i)).toList())))),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Text("${item['key1'] ?? ''}")),
      DataCell(Text("${item['key2'] ?? ''}")),
      DataCell(Text("${item['key3'] ?? ''}")),
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'key1', label: 'Key1', initial: "${item['key1'] ?? ''}"), AdminField(key: 'key2', label: 'Key2', initial: "${item['key2'] ?? ''}"), AdminField(key: 'key3', label: 'Key3', initial: "${item['key3'] ?? ''}")], onSubmit: (v) async { ref.invalidate(logsPaymentsProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Логи платежей', onDelete: () async { ref.invalidate(logsPaymentsProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
