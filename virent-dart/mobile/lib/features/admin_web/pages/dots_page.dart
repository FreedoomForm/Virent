...
class DotsPage extends ConsumerStatefulWidget {
  const DotsPage({super.key});
  @override
  ConsumerState<DotsPage> createState() => _DotsPageState();
}

class _DotsPageState extends ConsumerState<DotsPage> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(dotsProvider);
    return async.when(
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Dots', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить точку парковки', fields: const [AdminField(key: 'name', label: 'Название'), AdminField(key: 'lat', label: 'Широта'), AdminField(key: 'lng', label: 'Долгота')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/dots', values, dotsProvider); }),
                        icon: const Icon(Icons.add, size: 14, color: Colors.white),
                        label: const Text('Добавить точку парковки', style: TextStyle(fontSize: 11, color: Colors.white)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['name', 'lat', 'lon', 'type', 'radius'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'name', label: 'Name'), AdminField(key: 'type', label: 'Type')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Text("${item['lat'] ?? ''}")),
      DataCell(Text("${item['lon'] ?? ''}")),
      DataCell(Text("${item['type'] ?? ''}")),
      DataCell(Text("${item['radius'] ?? ''}")),
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'name', label: 'Name', initial: "${item['name'] ?? ''}"), AdminField(key: 'lat', label: 'Lat', initial: "${item['lat'] ?? ''}"), AdminField(key: 'lon', label: 'Lon', initial: "${item['lon'] ?? ''}"), AdminField(key: 'type', label: 'Type', initial: "${item['type'] ?? ''}"), AdminField(key: 'radius', label: 'Radius', initial: "${item['radius'] ?? ''}"), AdminField(key: 'description', label: 'Description', initial: "${item['description'] ?? ''}")], onSubmit: (v) async { ref.invalidate(dotsProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Dot', onDelete: () async { ref.invalidate(dotsProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
