...
class PromoSeriesPage extends ConsumerStatefulWidget {
  const PromoSeriesPage({super.key});
  @override
  ConsumerState<PromoSeriesPage> createState() => _PromoSeriesPageState();
}

class _PromoSeriesPageState extends ConsumerState<PromoSeriesPage> {
      ...
      data: (items) {
        ...
        return Container(
          ...
          child: Column(
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Серии промокодов', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить серию промокодов', fields: const [AdminField(key: 'name', label: 'Название'), AdminField(key: 'count', label: 'Количество')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/promo-series', values, promoSeriesProvider); }),
                        icon: const Icon(Icons.add, size: 14, color: Colors.white),
                        label: const Text('Добавить серию промокодов', style: TextStyle(fontSize: 11, color: Colors.white)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['id', 'name', 'active'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'name', label: 'Название'), AdminField(key: 'active', label: 'Активка')], onApply: (v) async {})),
            ...
            columnSpacing: 24,
            horizontalMargin: 12,
                    headingRowColor: WidgetStateProperty.all(adminBgLight), columns: [const DataColumn(label: Text('')), const DataColumn(label: Text('ID')), const DataColumn(label: Text('Название')), const DataColumn(label: Text('Активка')), const DataColumn(label: Text('Действия'))], rows: pageItems.map<DataRow>((i) => _buildRow(context, ref, i)).toList())))),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    ...
    return DataRow(cells: [
      ...
      DataCell(Text("${item['name'] ?? item['title'] ?? ''}")),
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'name', label: 'Название', initial: "${item['name'] ?? item['title'] ?? ''}"), AdminField(key: 'active', label: 'Активка (0/1)', initial: active ? '1' : '0')], onSubmit: (v) async { ref.invalidate(promoSeriesProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Серия промокодов', onDelete: () async { ref.invalidate(promoSeriesProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
  ...
  Widget _buildBulkActionBar(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminBulkActionDialog(context, title: 'Удалить', message: 'Удалить выбранные серии?', selectedCount: _selectedIds.length, onConfirm: () async { _selectedIds.clear(); }), icon: const Icon(Icons.delete, size: 14, color: adminDanger), label: const Text('Удалить', style: TextStyle(color: adminDanger, fontSize: 11))),
        const Spacer(),
