...
class SettingsDriversPage extends ConsumerStatefulWidget {
  const SettingsDriversPage({super.key});
  @override
  ConsumerState<SettingsDriversPage> createState() => _SettingsDriversPageState();
}

class _SettingsDriversPageState extends ConsumerState<SettingsDriversPage> {
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Драйверы', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: Color(0xFF1B2A4E))),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: Color(0xFF868686))),
                      ]),
                      ElevatedButton.icon(
                      onPressed: () => showAdminFormDialog(context, title: 'Добавить запись', fields: const [AdminField(key: 'value', label: 'Value'), AdminField(key: 'description', label: 'Description'), AdminField(key: 'type', label: 'Type')], onSubmit: (v) async { ref.invalidate(settingsDriversProvider); }),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: Color(0xFF6D737A)), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: [AdminField(key: 'value', label: 'Value'), AdminField(key: 'description', label: 'Description'), AdminField(key: 'type', label: 'Type')], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: Color(0xFF6D737A)), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'value', label: 'Value'), AdminField(key: 'description', label: 'Description'), AdminField(key: 'type', label: 'Type')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'value', label: 'Value', initial: '${item[\'value\'] ?? \'\'}'), AdminField(key: 'description', label: 'Description', initial: '${item[\'description\'] ?? \'\'}'), AdminField(key: 'type', label: 'Type', initial: '${item[\'type\'] ?? \'\'}')], onSubmit: (v) async { ref.invalidate(settingsDriversProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: Color(0xFF467FD0)), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: Color(0xFF467FD0)))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Драйверы', onDelete: () async { ref.invalidate(settingsDriversProvider); }), icon: const Icon(Icons.delete, size: 12, color: Color(0xFFDF4759)), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: Color(0xFFDF4759)))),
      ])),
    ]);
  }
