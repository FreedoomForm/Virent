...
class TarirovPage extends ConsumerStatefulWidget {
  const TarirovPage({super.key});
  @override
  ConsumerState<TarirovPage> createState() => _TarirovPageState();
}

class _TarirovPageState extends ConsumerState<TarirovPage> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(tarirovProvider);
    return async.when(
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Тарирование', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить запись тарирования', fields: const [AdminField(key: 'scooter_id', label: 'ID самоката'), AdminField(key: 'value', label: 'Значение')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/tarirov', values, tarirovProvider); }),
                        icon: const Icon(Icons.add, size: 14, color: Colors.white),
                        label: const Text('Добавить запись тарирования', style: TextStyle(fontSize: 11, color: Colors.white)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['id', 'joan', 'volume', 'cruising_range', 'cruising_time'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'joan', label: 'Joan'), AdminField(key: 'volume', label: 'Volume')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Text("${item['joan'] ?? ''}")),
      DataCell(Text("${item['volume'] ?? ''}")),
      DataCell(Text("${item['cruising_range'] ?? ''}")),
      DataCell(Text("${item['cruising_time'] ?? ''}")),
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'joan', label: 'Joan', initial: "${item['joan'] ?? ''}"), AdminField(key: 'volume', label: 'Volume', initial: "${item['volume'] ?? ''}"), AdminField(key: 'cruising_range', label: 'Cruising range', initial: "${item['cruising_range'] ?? ''}"), AdminField(key: 'cruising_time', label: 'Cruising time', initial: "${item['cruising_time'] ?? ''}")], onSubmit: (v) async { ref.invalidate(tarirovProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Тарирование', onDelete: () async { ref.invalidate(tarirovProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
