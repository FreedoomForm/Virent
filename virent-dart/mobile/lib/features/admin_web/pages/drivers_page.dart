                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Row(children: [
                      ...
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'value', label: 'Value'), AdminField(key: 'type', label: 'Type')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'value', label: 'Value', initial: "${item['value'] ?? ''}"), AdminField(key: 'description', label: 'Description', initial: "${item['description'] ?? ''}"), AdminField(key: 'type', label: 'Type', initial: "${item['type'] ?? ''}")], onSubmit: (v) async { ref.invalidate(driversProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Драйвер', onDelete: () async { ref.invalidate(driversProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
