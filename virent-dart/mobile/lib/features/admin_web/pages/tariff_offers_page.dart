...
class TariffOffersPage extends ConsumerStatefulWidget {
  const TariffOffersPage({super.key});
  @override
  ConsumerState<TariffOffersPage> createState() => _TariffOffersPageState();
}

class _TariffOffersPageState extends ConsumerState<TariffOffersPage> {
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Row(children: [
                      ...
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'admin_name', label: 'Название в админке'), AdminField(key: 'app_name', label: 'Название в приложении')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'admin_name', label: 'Название в админке', initial: "${item['admin_name'] ?? ''}"), AdminField(key: 'app_name', label: 'Название в приложении', initial: "${item['app_name'] ?? ''}"), AdminField(key: 'hold', label: 'Hold', initial: "${item['hold'] ?? ''}")], onSubmit: (v) async { ref.invalidate(tariffsListProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Тариф', onDelete: () async { ref.invalidate(tariffsListProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
