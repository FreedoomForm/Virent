...
class BillingReceiptsPage extends ConsumerStatefulWidget {
  const BillingReceiptsPage({super.key});
  @override
  ConsumerState<BillingReceiptsPage> createState() => _BillingReceiptsPageState();
}

class _BillingReceiptsPageState extends ConsumerState<BillingReceiptsPage> {
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Чеки', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить квитанцию', fields: const [AdminField(key: 'client_id', label: 'ID клиента'), AdminField(key: 'amount', label: 'Сумма'), AdminField(key: 'description', label: 'Описание')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/receipts', values, billingReceiptsProvider); }),
                        icon: const Icon(Icons.add, size: 14, color: Colors.white),
                        label: const Text('Добавить квитанцию', style: TextStyle(fontSize: 11, color: Colors.white)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['id', 'uuid', 'provider_uuid', 'bill', 'status', 'client', 'amount'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'uuid', label: 'Uuid'), AdminField(key: 'provider_uuid', label: 'Provider uuid'), AdminField(key: 'status', label: 'Status')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Text("${item['provider_uuid'] ?? ''}")),
      DataCell(Text("${item['bill'] ?? ''}")),
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'uuid', label: 'Uuid', initial: "${item['uuid'] ?? ''}"), AdminField(key: 'provider_uuid', label: 'Provider uuid', initial: "${item['provider_uuid'] ?? ''}"), AdminField(key: 'status', label: 'Status', initial: "${item['status'] ?? ''}"), AdminField(key: 'amount', label: 'Amount', initial: "${item['amount'] ?? ''}")], onSubmit: (v) async { ref.invalidate(billingReceiptsProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Чек', onDelete: () async { ref.invalidate(billingReceiptsProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
