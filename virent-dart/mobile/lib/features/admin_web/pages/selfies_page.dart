...
class SelfiesPage extends ConsumerStatefulWidget {
  const SelfiesPage({super.key});
  @override
  ConsumerState<SelfiesPage> createState() => _SelfiesPageState();
}

class _SelfiesPageState extends ConsumerState<SelfiesPage> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(selfiesProvider);
    return async.when(
      ...
      data: (items) {
        ...
        return Container(
          ...
          child: Column(
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Селфи', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить селфи', fields: const [AdminField(key: 'client_id', label: 'ID клиента'), AdminField(key: 'url', label: 'URL')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/selfies', values, selfiesProvider); }),
                        icon: const Icon(Icons.add, size: 14, color: Colors.white),
                        label: const Text('Добавить селфи', style: TextStyle(fontSize: 11, color: Colors.white)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['id', 'photo', 'verified'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'client_id', label: 'ID клиента'), AdminField(key: 'verified', label: 'Проверено')], onApply: (v) async {})),
            ...
            columnSpacing: 24,
            horizontalMargin: 12,
                    headingRowColor: WidgetStateProperty.all(adminBgLight), columns: [const DataColumn(label: Text('')), const DataColumn(label: Text('ID')), const DataColumn(label: Text('Фото')), const DataColumn(label: Text('Проверено')), const DataColumn(label: Text('Действия'))], rows: pageItems.map<DataRow>((i) => _buildRow(context, ref, i)).toList())))),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    final verified = item['verified'] == true || item['verified'] == 1 || item['verified'] == '1' || item['is_verified'] == true || item['is_verified'] == 1;
    final photo = "${item['photo'] ?? item['url'] ?? item['image'] ?? ''}";
    return DataRow(cells: [
      ...
      DataCell(photo.isNotEmpty
          ? Image.network(photo, width: 36, height: 36, fit: BoxFit.cover, errorBuilder: (ctx, _, __) => const Icon(Icons.broken_image, size: 18, color: adminTextGray))
          : const Icon(Icons.person, size: 18, color: adminTextGray)),
      DataCell(Icon(verified ? Icons.check_circle : Icons.cancel, size: 16, color: verified ? adminSuccess : adminDanger)),
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'verified', label: 'Проверено (0/1)', initial: verified ? '1' : '0')], onSubmit: (v) async { ref.invalidate(selfiesProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Селфи', onDelete: () async { ref.invalidate(selfiesProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
  ...
  Widget _buildBulkActionBar(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminBulkActionDialog(context, title: 'Удалить', message: 'Удалить выбранные селфи?', selectedCount: _selectedIds.length, onConfirm: () async { _selectedIds.clear(); }), icon: const Icon(Icons.delete, size: 14, color: adminDanger), label: const Text('Удалить', style: TextStyle(color: adminDanger, fontSize: 11))),
        const Spacer(),
