...
class AdminContactsPage extends ConsumerStatefulWidget {
  const AdminContactsPage({super.key});
  @override
  ConsumerState<AdminContactsPage> createState() => _AdminContactsPageState();
}

class _AdminContactsPageState extends ConsumerState<AdminContactsPage> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(adminContactsProvider);
    return async.when(
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Контакты', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить контакт', fields: const [AdminField(key: 'name', label: 'Имя'), AdminField(key: 'phone', label: 'Телефон')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/contacts', values, adminContactsProvider); }),
                        icon: const Icon(Icons.add, size: 14, color: Colors.white),
                        label: const Text('Добавить контакт', style: TextStyle(fontSize: 11, color: Colors.white)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['city', 'phone', 'email', 'telegram', 'whatsapp', 'year', 'company'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'city', label: 'City'), AdminField(key: 'phone', label: 'Phone'), AdminField(key: 'company', label: 'Company')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'city', label: 'City', initial: "${item['city'] ?? ''}"), AdminField(key: 'phone', label: 'Phone', initial: "${item['phone'] ?? ''}"), AdminField(key: 'email', label: 'Email', initial: "${item['email'] ?? ''}"), AdminField(key: 'telegram', label: 'Telegram', initial: "${item['telegram'] ?? ''}"), AdminField(key: 'whatsapp', label: 'Whatsapp', initial: "${item['whatsapp'] ?? ''}"), AdminField(key: 'company', label: 'Company', initial: "${item['company'] ?? ''}")], onSubmit: (v) async { ref.invalidate(adminContactsProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Контакт', onDelete: () async { ref.invalidate(adminContactsProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
