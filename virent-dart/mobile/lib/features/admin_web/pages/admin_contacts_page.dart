...
class AdminContactsPage extends ConsumerWidget {
            ...
            child: Column(
              children: [
                Container(
                  ...
                  child: const Row(
                    children: [
                      Expanded(child: Text('City', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600))),
                      Expanded(child: Text('Phone', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600))),
                      Expanded(child: Text('Email', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600))),
                      Expanded(child: Text('Telegram', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600))),
                      Expanded(child: Text('Whatsapp', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600))),
                      Expanded(child: Text('Faq', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600))),
                ...
                Expanded(
                  ref.watch(adminContactsProvider).when(
                    loading: () => const Center(child: CircularProgressIndicator()),
                    error: (e, _) => Center(child: Text("Ошибка: $e")),
                    data: (items) => ListView(
                      children: items.map((item) => _contactRowFromItem(item)).toList(),
                    ),
                  ),
                ),
                const Divider(height: 1),
  ...
  Widget _contactRow(String city, String phone, String email, String tg, String wp, String faq, String company) {
    return Container(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Expanded(child: Text(city, style: const TextStyle(fontSize: 11))),
          Expanded(child: Text(phone, style: const TextStyle(fontSize: 11))),
          Expanded(child: Text(email, style: const TextStyle(fontSize: 11, color: Color(0xFF7B68EE)))),
          Expanded(child: Text(tg, style: const TextStyle(fontSize: 11))),
          Expanded(child: Text(wp, style: const TextStyle(fontSize: 11))),
          Expanded(child: Text(faq, style: const TextStyle(fontSize: 11))),
  ...
  Widget _contactRowFromItem(Map<String, dynamic> item) {
    return _contactRow(
      item['city']?.toString() ?? '',
      item['phone']?.toString() ?? '',
      item['email']?.toString() ?? '',
