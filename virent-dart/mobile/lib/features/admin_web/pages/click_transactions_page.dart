...
class ClickTransactionsPage extends ConsumerStatefulWidget {
  const ClickTransactionsPage({super.key});
  @override
  ConsumerState<ClickTransactionsPage> createState() => _ClickTransactionsPageState();
}

class _ClickTransactionsPageState extends ConsumerState<ClickTransactionsPage> {
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Транзакции CLICK', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить транзакцию Click', fields: const [AdminField(key: 'amount', label: 'Сумма'), AdminField(key: 'client_id', label: 'ID клиента')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/click', values, clickTransactionsProvider); }),
                        icon: const Icon(Icons.add, size: 14, color: Colors.white),
                        label: const Text('Добавить транзакцию Click', style: TextStyle(fontSize: 11, color: Colors.white)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['id', 'click_trans', 'click_paydoc', 'merchant_trans', 'amount', 'status', 'created'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'click_trans', label: 'Click trans'), AdminField(key: 'merchant_trans', label: 'Merchant trans'), AdminField(key: 'status', label: 'Status')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Text("${item['click_trans'] ?? ''}")),
      DataCell(Text("${item['click_paydoc'] ?? ''}")),
      DataCell(Text("${item['merchant_trans'] ?? ''}")),
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'click_trans', label: 'Click trans', initial: "${item['click_trans'] ?? ''}"), AdminField(key: 'merchant_trans', label: 'Merchant trans', initial: "${item['merchant_trans'] ?? ''}"), AdminField(key: 'amount', label: 'Amount', initial: "${item['amount'] ?? ''}"), AdminField(key: 'status', label: 'Status', initial: "${item['status'] ?? ''}")], onSubmit: (v) async { ref.invalidate(clickTransactionsProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Транзакция CLICK', onDelete: () async { ref.invalidate(clickTransactionsProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
