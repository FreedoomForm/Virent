...
class AdminAgreementsPage extends ConsumerStatefulWidget {
  const AdminAgreementsPage({super.key});
  @override
  ConsumerState<AdminAgreementsPage> createState() => _AdminAgreementsPageState();
}

class _AdminAgreementsPageState extends ConsumerState<AdminAgreementsPage> {
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Договора', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить договор', fields: const [AdminField(key: 'name', label: 'Название'), AdminField(key: 'client_id', label: 'ID клиента')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/agreements', values, adminAgreementsProvider); }),
                        icon: const Icon(Icons.add, size: 14, color: Colors.white),
                        label: const Text('Добавить договор', style: TextStyle(fontSize: 11, color: Colors.white)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['document_type', 'file', 'url_label', 'html_file'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'document_type', label: 'Document type'), AdminField(key: 'file', label: 'File'), AdminField(key: 'url_label', label: 'Url lable')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Text("${item['file'] ?? ''}")),
      DataCell(Text("${item['url_label'] ?? ''}")),
      DataCell(Text("${item['html_file'] ?? ''}")),
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'document_type', label: 'Document type', initial: "${item['document_type'] ?? ''}"), AdminField(key: 'file', label: 'File', initial: "${item['file'] ?? ''}"), AdminField(key: 'url_label', label: 'Url lable', initial: "${item['url_label'] ?? ''}"), AdminField(key: 'html_file', label: 'Html file', initial: "${item['html_file'] ?? ''}")], onSubmit: (v) async { ref.invalidate(adminAgreementsProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Договор', onDelete: () async { ref.invalidate(adminAgreementsProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
