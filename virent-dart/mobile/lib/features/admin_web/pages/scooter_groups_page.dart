...
class ScooterGroupsPage extends ConsumerStatefulWidget {
  const ScooterGroupsPage({super.key});
  @override
  ConsumerState<ScooterGroupsPage> createState() => _ScooterGroupsPageState();
}

class _ScooterGroupsPageState extends ConsumerState<ScooterGroupsPage> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(scooterGroupsProvider);
    return async.when(
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить группу самокатов', fields: const [AdminField(key: 'name', label: 'Название'), AdminField(key: 'description', label: 'Описание')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/settings/scooter-groups', values, scooterGroupsProvider); }),
                        icon: const Icon(Icons.add, size: 14, color: Colors.white),
                        label: const Text('Добавить группу самокатов', style: TextStyle(fontSize: 11, color: Colors.white)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['id', 'description', 'trigger_equation'], onExport: (fmt, fields) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'description', label: 'Description', initial: "${item['description'] ?? ''}"), AdminField(key: 'trigger_equation', label: 'Trigger equation', initial: "${item['trigger_equation'] ?? ''}")], onSubmit: (v) async { ref.invalidate(scooterGroupsProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Группа самокатов', onDelete: () async { ref.invalidate(scooterGroupsProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
