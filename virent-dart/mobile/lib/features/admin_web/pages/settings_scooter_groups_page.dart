...
class SettingsScooterGroupsPage extends ConsumerStatefulWidget {
  const SettingsScooterGroupsPage({super.key});
  @override
  ConsumerState<SettingsScooterGroupsPage> createState() => _SettingsScooterGroupsPageState();
}

class _SettingsScooterGroupsPageState extends ConsumerState<SettingsScooterGroupsPage> {
      ...
      data: (items) {
        ...
        return Container(
          ...
          child: Column(
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Группы самокатов', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                      onPressed: () => showAdminFormDialog(context, title: 'Добавить запись', fields: const [AdminField(key: 'description', label: 'Description'), AdminField(key: 'trigger_equation', label: 'Trigger equation')], onSubmit: (v) async { ref.invalidate(settingsScooterGroupsProvider); }),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: [AdminField(key: 'description', label: 'Description'), AdminField(key: 'trigger_equation', label: 'Trigger equation')], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'description', label: 'Description'), AdminField(key: 'trigger_equation', label: 'Trigger equation')], onApply: (v) async {})),
            ...
            columnSpacing: 24,
            horizontalMargin: 12,
                    headingRowColor: WidgetStateProperty.all(adminBgLight), columns: [const DataColumn(label: Text('')), const DataColumn(label: Text('id')), const DataColumn(label: Text('Description')), const DataColumn(label: Text('Trigger equation')), const DataColumn(label: Text('Действия'))], rows: pageItems.map((i) => _buildRow(context, ref, i)).toList())))),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Text('${item['trigger_equation'] ?? ''}')),
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'description', label: 'Description', initial: '${item[\'description\'] ?? \'\'}'), AdminField(key: 'trigger_equation', label: 'Trigger equation', initial: '${item[\'trigger_equation\'] ?? \'\'}')], onSubmit: (v) async { ref.invalidate(settingsScooterGroupsProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Группы самокатов', onDelete: () async { ref.invalidate(settingsScooterGroupsProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
