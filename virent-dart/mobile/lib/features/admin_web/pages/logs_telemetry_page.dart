...
class LogsTelemetryPage extends ConsumerStatefulWidget {
  const LogsTelemetryPage({super.key});
  @override
  ConsumerState<LogsTelemetryPage> createState() => _LogsTelemetryPageState();
}

class _LogsTelemetryPageState extends ConsumerState<LogsTelemetryPage> {
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Логи телеметрии', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['car_id', 'gosnomer', 'speed', 'battery', 'lat', 'lon'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'car_id', label: 'CarId'), AdminField(key: 'gosnomer', label: 'Gosnomer')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'car_id', label: 'CarId', initial: "${item['car_id'] ?? ''}"), AdminField(key: 'gosnomer', label: 'Gosnomer', initial: "${item['gosnomer'] ?? ''}")], onSubmit: (v) async { ref.invalidate(logsTelemetryProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Логи телеметрии', onDelete: () async { ref.invalidate(logsTelemetryProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
