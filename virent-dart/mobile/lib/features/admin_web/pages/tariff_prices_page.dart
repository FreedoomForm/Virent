...
class TariffPricesPage extends ConsumerStatefulWidget {
  const TariffPricesPage({super.key});
  @override
  ConsumerState<TariffPricesPage> createState() => _TariffPricesPageState();
}

class _TariffPricesPageState extends ConsumerState<TariffPricesPage> {
      ...
      data: (items) {
        ...
        return Container(
          ...
          child: Column(
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Цены', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить цену тарифа', fields: const [AdminField(key: 'tariff_id', label: 'ID тарифа'), AdminField(key: 'price', label: 'Цена')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/tariff-prices', values, tariffPricesProvider); }),
                        icon: const Icon(Icons.add, size: 14, color: Colors.white),
                        label: const Text('Добавить цену тарифа', style: TextStyle(fontSize: 11, color: Colors.white)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['name', 'json', 'time_unit'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'name', label: 'Наименование'), AdminField(key: 'time_unit', label: 'Time unit')], onApply: (v) async {})),
            ...
            columnSpacing: 24,
            horizontalMargin: 12,
                    headingRowColor: WidgetStateProperty.all(adminBgLight), columns: [const DataColumn(label: Text('')), const DataColumn(label: Text('Наименование')), const DataColumn(label: Text('Json')), const DataColumn(label: Text('Time unit')), const DataColumn(label: Text('Действия'))], rows: pageItems.map<DataRow>((i) => _buildRow(context, ref, i)).toList())))),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Text("${item['json'] ?? ''}")),
      DataCell(Text("${item['time_unit'] ?? ''}")),
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'name', label: 'Наименование', initial: "${item['name'] ?? ''}"), AdminField(key: 'json', label: 'Json', initial: "${item['json'] ?? ''}"), AdminField(key: 'time_unit', label: 'Time unit', initial: "${item['time_unit'] ?? ''}")], onSubmit: (v) async { ref.invalidate(tariffPricesProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Цена', onDelete: () async { ref.invalidate(tariffPricesProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
