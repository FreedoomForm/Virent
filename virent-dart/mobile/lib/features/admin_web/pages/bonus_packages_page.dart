...
class BonusPackagesPage extends ConsumerStatefulWidget {
  const BonusPackagesPage({super.key});
  @override
  ConsumerState<BonusPackagesPage> createState() => _BonusPackagesPageState();
}

class _BonusPackagesPageState extends ConsumerState<BonusPackagesPage> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(bonusPackagesProvider);
    return async.when(
      ...
      data: (items) {
        ...
        return Container(
          ...
          child: Column(
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Пакеты бонусов', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить пакет', fields: const [AdminField(key: 'bonus', label: 'Bonus'), AdminField(key: 'cost', label: 'Cost'), AdminField(key: 'active', label: 'Active')], onSubmit: (v) async { ref.invalidate(bonusPackagesProvider); }),
                        icon: const Icon(Icons.add, size: 14, color: Colors.white),
                        label: const Text('Добавить пакет', style: TextStyle(fontSize: 11, color: Colors.white)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['bonus', 'cost', 'active'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'bonus', label: 'Bonus'), AdminField(key: 'cost', label: 'Cost'), AdminField(key: 'active', label: 'Active')], onApply: (v) async {})),
            ...
            columnSpacing: 24,
            horizontalMargin: 12,
                    headingRowColor: WidgetStateProperty.all(adminBgLight), columns: [const DataColumn(label: Text('')), const DataColumn(label: Text('Bonus')), const DataColumn(label: Text('Cost')), const DataColumn(label: Text('Active')), const DataColumn(label: Text('Действия'))], rows: pageItems.map<DataRow>((i) => _buildRow(context, ref, i)).toList())))),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'bonus', label: 'Bonus', initial: "${item['bonus'] ?? ''}"), AdminField(key: 'cost', label: 'Cost', initial: "${item['cost'] ?? ''}"), AdminField(key: 'active', label: 'Active', initial: "${item['active'] ?? ''}")], onSubmit: (v) async { ref.invalidate(bonusPackagesProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Пакет бонусов', onDelete: () async { ref.invalidate(bonusPackagesProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
