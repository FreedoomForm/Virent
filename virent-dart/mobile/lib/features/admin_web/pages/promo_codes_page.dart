...
class _PromoCodesPageState extends ConsumerState<PromoCodesPage> {
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Промокоды', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить промокод', fields: const [AdminField(key: 'code', label: 'Код'), AdminField(key: 'discount', label: 'Скидка'), AdminField(key: 'active', label: 'Активен')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/promo-codes', values, promoCodesProvider); }),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['id', 'code', 'bonus', 'group', 'expires'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'code', label: 'Код'), AdminField(key: 'group', label: 'Группа')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    final groupActive = item['group_active'] == true || item['group_active'] == 1 || item['group_active'] == '1';
    return DataRow(cells: [
      ...
      DataCell(Text("${item['bonus_gift'] ?? item['bonus'] ?? ''}")),
      DataCell(Text("${item['usage_remains'] ?? item['remains'] ?? ''}")),
      DataCell(Text("${item['promo_group'] ?? item['group'] ?? ''}")),
      DataCell(Icon(groupActive ? Icons.check_box : Icons.check_box_outline_blank, size: 14, color: groupActive ? adminSuccess : adminTextGray)),
      DataCell(Text("${item['expires'] ?? item['expires_at'] ?? ''}")),
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'code', label: 'Код', initial: "${item['code'] ?? ''}"), AdminField(key: 'bonus_gift', label: 'Бонус', initial: "${item['bonus_gift'] ?? item['bonus'] ?? ''}")], onSubmit: (v) async { ref.invalidate(promoCodesProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Промокод', onDelete: () async { ref.invalidate(promoCodesProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
