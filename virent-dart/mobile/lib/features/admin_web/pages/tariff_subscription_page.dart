...
class _TariffsSubscriptionsPageState extends ConsumerState<TariffsSubscriptionsPage> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(tariffSubscriptionProvider);
    return async.when(
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Подписочные тарифы', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить подписочный тариф', fields: const [AdminField(key: 'name', label: 'Название'), AdminField(key: 'price', label: 'Цена')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/tariff-subscriptions', values, tariffSubscriptionProvider); }),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['name', 'name_in_app', 'price', 'group', 'active'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'name', label: 'Name'), AdminField(key: 'group', label: 'Group'), AdminField(key: 'active', label: 'Active')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Text("${item['name_in_app'] ?? ''}")),
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'name', label: 'Name', initial: "${item['name'] ?? ''}"), AdminField(key: 'name_in_app', label: 'Name in app', initial: "${item['name_in_app'] ?? ''}"), AdminField(key: 'price', label: 'Price', initial: "${item['price'] ?? ''}"), AdminField(key: 'group', label: 'Group', initial: "${item['group'] ?? ''}"), AdminField(key: 'active', label: 'Active', initial: "${item['active'] ?? ''}")], onSubmit: (v) async { ref.invalidate(tariffSubscriptionProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Подписочный тариф', onDelete: () async { ref.invalidate(tariffSubscriptionProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
