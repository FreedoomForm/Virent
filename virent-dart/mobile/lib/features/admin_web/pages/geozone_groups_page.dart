...
class GeozoneGroupsPage extends ConsumerStatefulWidget {
  const GeozoneGroupsPage({super.key});
  @override
  ConsumerState<GeozoneGroupsPage> createState() => _GeozoneGroupsPageState();
}

class _GeozoneGroupsPageState extends ConsumerState<GeozoneGroupsPage> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(geozoneGroupsProvider);
    return async.when(
      ...
      data: (items) {
        ...
        return Container(
          ...
          child: Column(
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Группы геозон', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить группу геозон', fields: const [AdminField(key: 'name', label: 'Название'), AdminField(key: 'description', label: 'Описание')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/geozone-groups', values, geozoneGroupsProvider); }),
                        icon: const Icon(Icons.add, size: 14, color: Colors.white),
                        label: const Text('Добавить группу геозон', style: TextStyle(fontSize: 11, color: Colors.white)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['description', 'finish_geo'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'description', label: 'Description')], onApply: (v) async {})),
            ...
            columnSpacing: 24,
            horizontalMargin: 12,
                    headingRowColor: WidgetStateProperty.all(adminBgLight), columns: [const DataColumn(label: Text('')), const DataColumn(label: Text('Id')), const DataColumn(label: Text('Description')), const DataColumn(label: Text('FinishGeo')), const DataColumn(label: Text('Действия'))], rows: pageItems.map<DataRow>((i) => _buildRow(context, ref, i)).toList())))),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'description', label: 'Description', initial: "${item['description'] ?? ''}"), AdminField(key: 'finish_geo', label: 'FinishGeo', initial: "${item['finish_geo'] ?? ''}")], onSubmit: (v) async { ref.invalidate(geozoneGroupsProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Группа геозон', onDelete: () async { ref.invalidate(geozoneGroupsProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
