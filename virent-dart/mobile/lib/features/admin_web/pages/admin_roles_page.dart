...
class _AdminRolesPageState extends ConsumerState<AdminRolesPage> {
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Роли', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить роль', fields: const [AdminField(key: 'name', label: 'Название'), AdminField(key: 'permissions', label: 'Права')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/roles', values, adminRolesProvider); }),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['id', 'name', 'roles', 'companion', 'tech_key', 'api_token', 'permissions', 'admin'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'name', label: 'Имя'), AdminField(key: 'roles', label: 'Роли'), AdminField(key: 'permissions', label: 'Permissions')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Text("${item['companion'] ?? ''}")),
      DataCell(Text("${item['tech_key'] ?? ''}")),
      DataCell(Text("${item['api_token'] ?? ''}")),
      DataCell(Text("${item['permissions'] ?? ''}")),
      DataCell(Text("${item['admin'] ?? ''}")),
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'name', label: 'Имя', initial: "${item['name'] ?? ''}"), AdminField(key: 'roles', label: 'Роли', initial: "${item['roles'] ?? ''}"), AdminField(key: 'permissions', label: 'Permissions', initial: "${item['permissions'] ?? ''}", multiline: true)], onSubmit: (v) async { ref.invalidate(adminRolesProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Роль', onDelete: () async { ref.invalidate(adminRolesProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
