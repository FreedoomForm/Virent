                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('IoT устройства', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить команду IoT', fields: const [AdminField(key: 'scooter_id', label: 'ID самоката'), AdminField(key: 'command', label: 'Команда')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/iot/logs', values, iotLogsProvider); }),
                        icon: const Icon(Icons.add, size: 14, color: Colors.white),
                        label: const Text('Добавить команду IoT', style: TextStyle(fontSize: 11, color: Colors.white)),
