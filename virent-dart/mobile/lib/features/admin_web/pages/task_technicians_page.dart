...
class TaskTechniciansPage extends ConsumerStatefulWidget {
  const TaskTechniciansPage({super.key});
  @override
  ConsumerState<TaskTechniciansPage> createState() => _TaskTechniciansPageState();
}

class _TaskTechniciansPageState extends ConsumerState<TaskTechniciansPage> {
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Задачи техников', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: Color(0xFF1B2A4E))),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: Color(0xFF868686))),
                      ]),
                      ElevatedButton.icon(
                      onPressed: () => showAdminFormDialog(context, title: 'Добавить задачу', fields: const [AdminField(key: 'title', label: 'Title'), AdminField(key: 'technician', label: 'Technician'), AdminField(key: 'description', label: 'Description')], onSubmit: (v) async { ref.invalidate(techTasksProvider); }),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: Color(0xFF6D737A)), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: [AdminField(key: 'title', label: 'Title'), AdminField(key: 'technician', label: 'Technician'), AdminField(key: 'description', label: 'Description')], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: Color(0xFF6D737A)), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'title', label: 'Title'), AdminField(key: 'technician', label: 'Technician'), AdminField(key: 'description', label: 'Description')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Text('${item['create_by'] ?? ''}'))
      DataCell(Text('${item['create_time'] ?? ''}'))
      DataCell(Text('${item['done'] ?? ''}'))
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Задачи техников', onDelete: () async { ref.invalidate(techTasksProvider); }), icon: const Icon(Icons.delete, size: 12, color: Color(0xFFDF4759)), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: Color(0xFFDF4759)))),
      ])),
    ]);
  }
