...
class BillingDebtsPage extends ConsumerStatefulWidget {
  const BillingDebtsPage({super.key});
  @override
  ConsumerState<BillingDebtsPage> createState() => _BillingDebtsPageState();
}

class _BillingDebtsPageState extends ConsumerState<BillingDebtsPage> {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(billingDebtsProvider);
    return async.when(
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Долги', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                        const SizedBox(width: 12),
                        Text('Показано ${filtered.length} совпадений', style: const TextStyle(fontSize: 11, color: adminTextGray)),
                      ]),
                      ElevatedButton.icon(
                        onPressed: () => showAdminFormDialog(context, title: 'Добавить долг', fields: const [AdminField(key: 'client_id', label: 'ID клиента'), AdminField(key: 'amount', label: 'Сумма')], onSubmit: (values) async { await ref.read(genericCreateAction)('/admin/debts', values, billingDebtsProvider); }),
                        icon: const Icon(Icons.add, size: 14, color: Colors.white),
                        label: const Text('Добавить долг', style: TextStyle(fontSize: 11, color: Colors.white)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['id', 'client', 'order', 'amount', 'status', 'created'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'client', label: 'Client'), AdminField(key: 'order', label: 'Order'), AdminField(key: 'status', label: 'Status')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'client', label: 'Client', initial: "${item['client'] ?? ''}"), AdminField(key: 'order', label: 'Order', initial: "${item['order'] ?? ''}"), AdminField(key: 'amount', label: 'Amount', initial: "${item['amount'] ?? ''}"), AdminField(key: 'status', label: 'Status', initial: "${item['status'] ?? ''}")], onSubmit: (v) async { ref.invalidate(billingDebtsProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Долг', onDelete: () async { ref.invalidate(billingDebtsProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
