...
class TechnicianTasksPage extends ConsumerStatefulWidget {
  const TechnicianTasksPage({super.key});
  @override
  ConsumerState<TechnicianTasksPage> createState() => _TechnicianTasksPageState();
}

class _TechnicianTasksPageState extends ConsumerState<TechnicianTasksPage> {
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Задачи техников', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                    ...
                    Row(children: [
                      ...
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'title', label: 'Title'), AdminField(key: 'technician', label: 'Technician'), AdminField(key: 'finished', label: 'Завершен')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'title', label: 'Title', initial: "${item['title'] ?? ''}"), AdminField(key: 'technician', label: 'Technician', initial: "${item['technician'] ?? ''}"), AdminField(key: 'description', label: 'Description', initial: "${item['description'] ?? ''}", multiline: true), AdminField(key: 'finished', label: 'Завершен', initial: "${item['finished'] ?? ''}")], onSubmit: (v) async { ref.invalidate(techTasksProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Задача техника', onDelete: () async { ref.invalidate(techTasksProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
