...
class _AlertsPageState extends ConsumerState<AlertsPage> {
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['scooter_id', 'alert_type', 'time', 'status'], onExport: (fmt, fields) async {})),
                      IconButton(icon: const Icon(Icons.filter_list, size: 18, color: adminTextSecondary), tooltip: 'Фильтры', onPressed: () => showAdminFilterDialog(context, title: 'Фильтры', fields: const [AdminField(key: 'scooter_id', label: 'Scooter id'), AdminField(key: 'alert_type', label: 'Alert type'), AdminField(key: 'status', label: 'Status')], onApply: (v) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Icon(Icons.warning, size: 16, color: adminWarning)),
      DataCell(Text("${item['scooter_id'] ?? ''}")),
      DataCell(Text("${item['alert_type'] ?? ''}")),
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'scooter_id', label: 'Scooter id', initial: "${item['scooter_id'] ?? ''}"), AdminField(key: 'alert_type', label: 'Alert type', initial: "${item['alert_type'] ?? ''}"), AdminField(key: 'status', label: 'Status', initial: "${item['status'] ?? ''}")], onSubmit: (v) async { ref.invalidate(alertsListProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Тревога', onDelete: () async { ref.invalidate(alertsListProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
