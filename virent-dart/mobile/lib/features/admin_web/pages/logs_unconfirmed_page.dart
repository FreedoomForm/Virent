...
class LogsUnconfirmedPage extends ConsumerStatefulWidget {
  const LogsUnconfirmedPage({super.key});
  @override
  ConsumerState<LogsUnconfirmedPage> createState() => _LogsUnconfirmedPageState();
}

class _LogsUnconfirmedPageState extends ConsumerState<LogsUnconfirmedPage> {
                ...
                child: Row(
                  ...
                  children: [
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        const Text('Неподтвержденные', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w400, color: adminTextDark)),
                    ...
                    Row(children: [
                      IconButton(icon: const Icon(Icons.download, size: 18, color: adminTextSecondary), tooltip: 'Экспорт', onPressed: () => showAdminExportDialog(context, title: 'Экспорт', fields: ['phone', 'sms_code', 'try_count', 'create_time'], onExport: (fmt, fields) async {})),
  ...
  DataRow _buildRow(BuildContext context, WidgetRef ref, Map<String, dynamic> item) {
    return DataRow(cells: [
      ...
      DataCell(Row(children: [
        ...
        TextButton.icon(onPressed: () => showAdminFormDialog(context, title: 'Редактировать', fields: [AdminField(key: 'phone', label: 'Phone', initial: "${item['phone'] ?? ''}"), AdminField(key: 'sms_code', label: 'Sms code', initial: "${item['sms_code'] ?? ''}")], onSubmit: (v) async { ref.invalidate(logsUnconfirmedProvider); }, isEdit: true), icon: const Icon(Icons.edit, size: 12, color: adminInfo), label: const Text('Редактировать', style: TextStyle(fontSize: 10, color: adminInfo))),
        TextButton.icon(onPressed: () => showAdminDeleteDialog(context, name: 'Неподтвержденные', onDelete: () async { ref.invalidate(logsUnconfirmedProvider); }), icon: const Icon(Icons.delete, size: 12, color: adminDanger), label: const Text('Удалить', style: TextStyle(fontSize: 10, color: adminDanger))),
      ])),
    ]);
  }
