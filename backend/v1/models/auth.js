...
const auth = {
    ...
    adminLogin: async function(res, body, path) { // Admin login
        ...
        try {
            ...
            bcrypt.compare(adminPassword, admin.password, (err, result) => {
                ...
                let payload = { id: String(admin._id), api_key: apiKey, email: admin.email, role: 'admin' };
    ...
    adminCheckToken: function(req, res, next) { // Check the x-access-token from admin
        ...
        jwt.verify(token, jwtSecret, function(err, decoded) { // Verify the token
            ...
            req.admin.user_id = decoded.id || decoded.email; // for audit log
            req.admin.id = decoded.id || decoded.email;
            // Also set req.user for code that uses req.user (e.g. transactions admin endpoint)
            req.user = req.user || {};
