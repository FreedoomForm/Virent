...
const scooters = {
    ...
    scooterField: {
        ...
        status: "string",
        log: {},
        // --- Extended fields (scooter passport) ---
        ...
        battery_cycles: "int",
        last_battery_health_check: "date",
    ...
    registerScooter: async function(res, body, path) {
        ...
        try {
            ...
            let registerScooterDataField = {
                ...
                log: [],
                // --- Scooter passport fields (extended) ---
                serial_number: sanitize(body.serial_number) || `SN-${Date.now()}`,
                model: sanitize(body.model) || 'unknown',
                manufacturer: sanitize(body.manufacturer) || 'unknown',
                purchase_date: sanitize(body.purchase_date) ? new Date(sanitize(body.purchase_date)) : new Date(),
                purchase_price: parseFloat(sanitize(body.purchase_price)) || 0,
                firmware_version: sanitize(body.firmware_version) || '1.0.0',
                hardware_version: sanitize(body.hardware_version) || '1.0',
                mac_address: sanitize(body.mac_address) || null,
                imei: sanitize(body.imei) || null,
                sim_number: sanitize(body.sim_number) || null,
                total_distance_km: 0,
                total_rides: 0,
                last_maintenance_at: null,
                last_maintenance_note: null,
                next_maintenance_at: null,
                retired_at: null,
                retired_reason: null,
                max_speed_kmh: parseInt(sanitize(body.max_speed_kmh)) || 25,
                battery_capacity_wh: parseFloat(sanitize(body.battery_capacity_wh)) || 250,
                battery_cycles: 0,
                last_battery_health_check: null,
