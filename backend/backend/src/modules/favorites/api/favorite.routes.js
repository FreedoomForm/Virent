...
router.post('/', (req, res, next) => authModel.userCheckToken(req, res, next),
    async (req, res) => {
        try {
            const fav = await favService.add(req.user.id, req.body);
            res.status(201).json({ data: { type: 'success', favorite: fav }});
        } catch (e) { res.status(400).json({ errors: { detail: e.message }}); }
    });

router.delete('/:id', (req, res, next) => authModel.userCheckToken(req, res, next),
    async (req, res) => {
